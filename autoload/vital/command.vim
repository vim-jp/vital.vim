...
let g:vital#command#vital_dir =
\     exists('g:vitalizer#vital_dir') ? g:vitalizer#vital_dir :
\     get(g:, 'vital#command#vital_dir', expand('<sfile>:h:h:h:p'))

let s:DATA_DIR = s:FP.join(g:vital#command#vital_dir, 'data', 'vital')
...
function! s:git_dir() abort
  return s:FP.join(g:vital#command#vital_dir, '.git')
endfunction
...
function! s:get_changes() abort
  let changes_file = s:FP.join(g:vital#command#vital_dir, 'Changes')
...
function! s:install_module_files(module_files, plugin_name, to) abort
  ...
  for f in a:module_files
    ...
    let paths = globpath(g:vital#command#vital_dir . ',' . &runtimepath, pat, 1)
...
function! s:loader_files(plugin_name, to) abort
  ...
  for f in s:LOADER_FILES
    let from = s:FP.join(g:vital#command#vital_dir, f)
    if !filereadable(from)
...
function! vital#command#complete(arglead, cmdline, cursorpos) abort
  let options = ['--help']
  ...
  if args[0] =~# '^-'
    return filter(options, 'stridx(v:val, args[0]) != -1')
  endif
  let func = 'vital#command#complete_' . args[0]
  if !exists('*' . func)
    return []
  endif
  return call(func, [arglead, cmdline, cursorpos])
endfunction

function! vital#command#run(args) abort
  if empty(a:args) || a:args[0] =~# '^--help$'
    return vital#command#run_help(a:args)
  endif
  let func = 'vital#command#run_' . a:args[0]
  if !exists('*' . func)
    echohl ErrorMsg
    echo 'Vital: No subcommand ''' . a:args[0] . '''.'
    echohl None
    return vital#command#run_help([])
  endif
  return call(func, [a:args[1:]])
endfunction

" Dispatches to ':Vital {subcmd} -help'
function! vital#command#run_help(args) abort
  if empty(a:args)
    echo 'Usage :Vital {subcommand} ...'
    echo '  See '':Vital {subcommand} --help'' for each command help'
    return
  endif
  let func = 'vital#command#run_' . a:args[0]
  if !exists('*' . func)
  ...
  return call(func, [['--help']])
endfunction

function! vital#command#complete_bundle(arglead, cmdline, cursorpos) abort
...
function! vital#command#run_bundle(args) abort
  try
    call s:check_system()
  catch
  ...
  for option in options
    if option =~# '^--help'
      echo 'Usage :Vital bundle [options ...] {target-dir} [module ...]'
      return
  ...
  try
    let result = s:do_bundle(name, to, modules, hash)
...
function! s:do_bundle(name, to, modules, hash) abort
