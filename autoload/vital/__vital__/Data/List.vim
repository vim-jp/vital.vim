...
function! s:sort_by(list, unary_f) abort
  let scope = {'unary_func': a:unary_f}
  ...
  function! scope.compare_with(x, y) abort dict
    let l:Call = s:_get_caller(self.unary_func)
    let x = l:Call(self.unary_func, [a:x])
    let y = l:Call(self.unary_func, [a:y])
