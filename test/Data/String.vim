...
function! s:suite.trim()
  ...
  for n in range(0x09, 0x0d)
    let c = nr2char(n)
    call s:assert.equals(s:String.trim(c . "hello" . c), c . "hello" . c, printf('trim 0x%02x (without SPC(0x20))', n))
    call s:assert.equals(s:String.trim(c . "  hello  " . c), c . "  hello  " . c, printf('trim 0x%02x (with SPC(0x20))', n))
  endfor
endfunction
